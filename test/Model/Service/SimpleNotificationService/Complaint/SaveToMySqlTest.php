<?php
namespace MonthlyBasis\SimpleEmailTest\Model\Service\SimpleNotificationService\Complaint;
...
class SaveToMySqlTest extends TestCase
{
    ...
    {
        $this->complaintLogTableMock = $this->createMock(
            SimpleEmailServiceTable\ComplaintLog::class
        );

        $this->saveToMySqlService = new SimpleEmailServiceService\SimpleNotificationService\Complaint\SaveToMySql(
            $this->complaintLogTableMock
        );
    }
    ...
    public function test_saveToMySql_oneEmailAddress_oneInsert()
    {
        $this->complaintLogTableMock
            ->expects($this->once())
            ->method('insert')
            ->with('complaint@simulator.amazonses.com')
            ;
        ...
        $message['Message'] = '{"complaint":{"complainedRecipients":[{"emailAddress":"complaint@simulator.amazonses.com","action":"failed"}]}}';
     ...
     * complaint email addresses. So, if we can receive this message while
